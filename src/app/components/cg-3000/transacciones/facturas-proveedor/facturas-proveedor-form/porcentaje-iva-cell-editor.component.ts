...
export interface PorcentajeIvaCombo {
  id: number;          // idPorIva
  label: string;       // ej: "2 - IVA 12% (12%)"
  codigoIva: number;
  porcentaje: number;
  descripcion: string;
}
...
@Component({
  selector: 'app-porcentaje-iva-cell-editor',
  standalone: true,
...
export class PorcentajeIvaCellEditorComponent implements ICellEditorAngularComp {

  @ViewChild('input') input!: ElementRef<HTMLInputElement>;
  @ViewChildren('itemRef') items!: QueryList<ElementRef<HTMLLIElement>>;

  private params!: ICellEditorParams & { porcentajesIva?: PorcentajeIvaCombo[] };

  porcentajesIva: PorcentajeIvaCombo[] = [];
  filtered: PorcentajeIvaCombo[] = [];
  value: number | null = null;
  search = '';
  selectedIndex = 0;
  columnWidth = 0;

  agInit(params: ICellEditorParams & { porcentajesIva?: PorcentajeIvaCombo[] }): void {
    this.params = params;

    this.porcentajesIva = params.porcentajesIva ?? [];
    this.filtered = [...this.porcentajesIva];

    const currentValue = params.value ?? null;
    this.value = currentValue === 0 ? null : currentValue;

    const sel = this.porcentajesIva.find(t => t.id === this.value);
  ...
  onSearchChange(): void {
    ...
    this.filtered = this.porcentajesIva.filter(t =>
  ...
  refresh(params: ICellEditorParams & { porcentajesIva?: PorcentajeIvaCombo[] }): boolean {
    this.params = params;
    this.porcentajesIva = params.porcentajesIva ?? [];
    this.filtered = [...this.porcentajesIva];
