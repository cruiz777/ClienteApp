// src/app/components/.../plan-cuenta-cell-editor.component.ts
...
interface CuentaItem {
  ...
  label: string;   // "CuentaPresentacion - NombreCuenta"
}
...
@Component({
  ...
  template: `
    <div class="cuenta-editor" [style.width.px]="columnWidth" >
      ...
      <ul class="cuenta-list" *ngIf="filtered.length > 0">
        <li *ngFor="let c of filtered; let i = index"
            #itemRef
  ...
  `,
  styles: [`
    .cuenta-editor { position: relative; width: 100%; }
...
export class PlanCuentaCellEditorComponent implements ICellEditorAngularComp {
  ...
  private params!: ICellEditorParams & { cuentas?: CuentaItem[] };
  ...
  agInit(params: ICellEditorParams & { cuentas?: CuentaItem[] }): void {
    this.params = params;
    ...
    const sel = this.cuentas.find(c => c.id === this.value);
    this.search = sel ? sel.label : '';

    // posiciona el índice sobre la cuenta actual si existe
    if (sel) {
      const idx = this.filtered.findIndex(c => c.id === sel.id);
