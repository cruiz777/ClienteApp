...
interface DecimoTerceroDetalle {
  posicion: number;
  local: string;
  noAfiliacion: number;
  cedula: string;
...
export class DecimoTerceroComponent implements OnInit {
  form!: FormGroup;
  ...
  dataSource: DecimoTerceroDetalle[] = [];
  ...
  cargarDatosMock(): void {
    this.dataSource = [
      ...
      },
      {
        ...
        nombre: 'Morán Poma Rocío Solange Danitza',
        numeroDias: 360,
        decimoTercero: 470,
        fechaIng: '07/03/2022',
        fechaSal: '28/02/2022',
  ...
  calcular(): void {
    ...
    const total = this.dataSource.reduce((acc, item) => acc + item.decimoTercero, 0);

    this.resumen.totalDecimoTercero = total;
    this.resumen.liquidoRecibir =
      this.resumen.totalDecimoTercero -
  ...
  grabar(): void {
    console.log('Grabar información Décimo Tercero', {
  ...
  exportar(): void {
    console.log('Exportar reporte Décimo Tercero');
  }
