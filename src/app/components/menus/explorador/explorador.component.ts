...
export class ExploradorComponent implements OnInit {
  ...
  onGridReady(params: any): void {
    this.cargarDatos();
    // No llamar setDatasource aquí, solo asignamos datasource en template
  }
...
cargarDatos() {
  ...
  this.clienteService.getClientesPaginados(1, 20000, filtros).subscribe({
    next: (res: any) => {
      ...
      this.totalRegistros = res.count ?? this.rowData.length; // 👈 usa el total real
  ...
  aplicarFiltros() {
    this.datasource = this.crearDatasource();
    // Cambia la datasource para que ag-Grid refresque automáticamente
  }
  ...
  onPageSizeInput(val: string | number) {
  ...
  return {
    getRows: (params: IGetRowsParams) => {
      ...
      this.clienteService.getClientesPaginados(pageNumber, this.pageSize, filtros)
        .subscribe({
          next: res => {
            const clientes = res.data;
            this.totalRegistros = res.count;

            // Devuelve los datos y total para que ag-Grid sepa el tamaño
            params.successCallback(clientes, this.totalRegistros);

            // Opcional: actualiza el arreglo visible (por si usas export, etc)
            this.clientesFiltrados = clientes;
          },
          error: () => {
            // En caso de error, puedes avisar y pasar vacío
            this.mostrarAlerta('Error cargando clientes', 'Error');
...
exportar(tipo: 'excel' | 'pdf'): void {
  ...
  // Encabezados y columnas según tu grid
  const headers = [
  ...
  // Mapeo + formateos (fecha / teléfono)
